...
const PatientAdmissionList = () => {
    ...
    const [statusFilter, setStatusFilter] = useState('Twin');
    ...
    const fetchClients = async () => {
        try {
            setLoading(true);
            const url = `${API_BASE_URL}/api/patientadmission/clients${statusFilter !== 'ALL' ? `?status=${statusFilter}` : ''}`;
    ...
    const filterClients = () => {
        ...
        const filtered = clients.filter(client =>
            ...
            client.care_center?.toLowerCase().includes(term) ||
    ...
    const handleTriggerBilling = async (patientName) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/patientadmission/generate-invoice/${encodeURIComponent(patientName)}`, {
                method: 'POST'
            });
            const data = await response.json();
            ...
            <div className="flex-1 flex flex-col overflow-hidden p-6">
                ...
                <div className="flex-shrink-0 mb-4">
                    <div className="flex justify-between items-center mb-2">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-800">Patient Admission Clients</h1>
                            <p className="text-gray-600 text-sm">Manage patient admission services and billing</p>
                        </div>
                        <div className="flex gap-2">
                            <button
                                onClick={() => navigate('/patientadmission/create')}
                            ...
                            <button
                                onClick={() => navigate('/patientadmission/billing-preview')}
                    ...
                    <div className="flex gap-3 items-center">
                        <div className="flex-1 relative">
                            ...
                            <input
                                type="text"
                                placeholder="Search by patient name, care_center, or pain point..."
                                value={searchTerm}
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => setStatusFilter('Twin')}
                                className={`px-4 py-2 rounded-lg ${statusFilter === 'Twin' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                            >
                                Twin
                            </button>
                            <button
                                onClick={() => setStatusFilter('Single')}
                                className={`px-4 py-2 rounded-lg ${statusFilter === 'Single' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                            >
                                Single
                            </button>
                            <button
