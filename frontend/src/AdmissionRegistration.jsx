...
import {
    ...
    Upload, Search, Bed, Edit3, Plus, Trash2
} from 'lucide-react';
...
const AdmissionRegistration = ({ generateMemberId, onSearch, currentStep, onStepChange }) => {
    ...
    const fetchAdmissions = async () => {
        ...
        try {
            const res = await api.get('/patient-admission/view');
        ...
        const fetchData = async () => {
            try {
                ...
                const [bedsRes, patientsRes] = await Promise.all([
                    api.get('/api/beds'),
                    api.get('/search_data?limit=1000')
                ]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Send as { rows: [...] }
            await api.post('/patient-admission/save', { rows: formattedRows });
    ...
    const AdmissionRegistration = ({ generateMemberId, onSearch, currentStep, onStepChange }) => {
        ...
        const availablePatientOptions = React.useMemo(() => {
            if (!patients || !patients.length) return [];
            const { idIdx } = headerIndices;

            return patients.map((row, i) => {
                let id = '', name = 'Unknown';
                if (Array.isArray(row)) {
                    if (idIdx !== -1) id = String(row[idIdx] || '').trim();
                    // Look specifically for "Patient Name" column (index 3 typically)
                    const patientNameIdx = patientHeaders.findIndex(h => /^patient\s*name$/i.test(String(h).trim()));
                    if (patientNameIdx !== -1 && row[patientNameIdx]) {
                        name = String(row[patientNameIdx]).trim();
                    }
                } else if (typeof row === 'object') {
                    const keys = Object.keys(row);
                    const idKey = keys.find(k => /member.*id|key/i.test(k));
                    // Look specifically for "Patient Name" key
                    const nameKey = keys.find(k => /^patient\s*name$/i.test(k));
                    if (idKey) id = String(row[idKey]).trim();
        ...
        }, [patients, headerIndices, occupiedMemberIds, patientHeaders]);
        ...
        const handleMemberSelect = (index, e) => {
            ...
            setPatientAdmissionList(prev => {
                ...
                const row = selectedOption.original;
                // SMART PARSER implementation with specific Patient Name extraction
                let rowVals = [];
                if (Array.isArray(row)) rowVals = row;
                else if (typeof row === 'object') rowVals = Object.values(row);
                rowVals = rowVals.map(v => String(v).trim());

                let foundId = '', foundGender = '', foundEmail = '', foundPhone = '', foundDob = '', foundPatientName = '', foundAge = '';
                ...
                // Extract Patient Name specifically from the correct column
                if (Array.isArray(row)) {
                    const patientNameIdx = patientHeaders.findIndex(h => /^patient\s*name$/i.test(String(h).trim()));
                    if (patientNameIdx !== -1 && row[patientNameIdx]) {
                        foundPatientName = String(row[patientNameIdx]).trim();
                    }
                } else if (typeof row === 'object') {
                    const keys = Object.keys(row);
                    const patientNameKey = keys.find(k => /^patient\s*name$/i.test(k));
                    if (patientNameKey && row[patientNameKey]) {
                        foundPatientName = String(row[patientNameKey]).trim();
                    }
                }
